...
def json_wthr_data(selec_yr, selec_mnth):
    yr = selec_yr
    mnth = selec_mnth
    selec_df = sta_wthr.copy()

    # rip selected month and year from the weather station data
    selec_df = selec_df[selec_df['month'] == mnth]
    selec_df = selec_df[selec_df['year'] == yr]
...
# make a column layout of Column(widget_name) and plot, and add it to the current document
